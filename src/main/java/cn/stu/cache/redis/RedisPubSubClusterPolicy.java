...
import cn.stu.cache.CacheProviderHolder;
import cn.stu.cache.Command;
 ...
 * 该策略器使用 cache.properties 中的 redis 配置自行保持两个到 redis 的连接用于发布和订阅消息（并在失败时自动重连）
 * @author zhanghanlin
 */
public class RedisPubSubClusterPolicy extends JedisPubSub implements ClusterPolicy {
    ...
    public RedisPubSubClusterPolicy(String channel, Properties props){
        ...
        if("sentinel".equalsIgnoreCase(props.getProperty("mode"))){
            ...
            String masterName = props.getProperty("cluster_name", "cache");
