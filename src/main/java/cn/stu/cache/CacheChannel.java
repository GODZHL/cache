...
/**
 * Cache Channel, the ZhCache methods explored to developers
 *
 * @author zhanghanlin
 */
public abstract class CacheChannel implements Closeable , AutoCloseable {
	...
	public CacheChannel(ZhCacheConfig config, CacheProviderHolder holder) {
		this.config = config;
		this.holder = holder;
	...
	/**
	 * Write data to cache with expired setting
	 *
	...
	/**
	 * <p>获取二级缓存的接口，该方法可用来直接获取 ZhCache 底层的 Redis 客户端实例</p>
	 * <p>方法如下：</p>
	 * <code>
	 *     CacheChannel channel = ZhCache.getChannel();
